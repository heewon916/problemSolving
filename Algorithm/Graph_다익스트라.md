...
- ⭐핵심: `dist[v]` = 시작정점 `s`에서 `v`까지의 최소 비용  
...
> ✅ 풀이과정
1. dist배열 선언
   1. ⭐시작정점의 거리 0 설정, 나머지는 `Integer.MAX_VALUE`
2. 우선순위 큐에서 현재까지 거리가 가장 짧은 정점 선택 
   1. 이미 확정된 최단 경로는 다시 갱신 x
3. 인접한 노드에 대해 거리를 갱신한다.
   1. `dist[adj] = min(dist[adj], dist[cur] + w)`
   2. 더 
